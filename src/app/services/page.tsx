...
export const metadata = {
  title: "Services · Intrastack",
  description:
    "Focused service lines spanning cloud, data, AI, security, and the people who deliver them.",
...
export default function ServicesHub() {
  ...
  return (
    <main className="flex flex-col flex-1 pt-20">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-24 lg:py-32">
          ...
          <Reveal direction="left" delay={250}>
            <p className="mt-7 max-w-2xl text-lg text-muted-foreground leading-relaxed">
              Focused service lines spanning cloud, data, AI, security, and the people who deliver them —
              concrete deliverables, senior engineers, and a credible hand-off. Pick the one that fits the
              problem.
            </p>
          </Reveal>
        </div>
      ...
      {/* GRID */}
      <section className="w-full bg-accent py-24 px-6" aria-labelledby="services-grid-heading">
        <h2 id="services-grid-heading" className="sr-only">All services</h2>
        <RevealGroup direction="up" stagger={80} className="mx-auto max-w-7xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((s) => (
            <Link
              key={s.slug}
          ...
          <Reveal direction="right">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-foreground leading-tight">
                Not sure which one <span className="italic font-serif text-secondary">fits?</span>
              </h2>
              <p className="mt-3 text-muted-foreground">
                30-min call with a senior engineer · we&apos;ll point you to the right shape.
              </p>
            </div>
          </Reveal>
          <Reveal direction="left" delay={150} className="shrink-0">
            <Link
              href="/contact"
